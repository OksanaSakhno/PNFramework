...
public class MenuNextPrevSort extends Component {
	...
	public static MenuNextPrevSort getMenuComp() {
		MenuNextPrevSort menuSort = PageFactory.initElements(
				Component.getDriver(), MenuNextPrevSort.class);
		return menuSort;
	}
	...
	public void clickAllProductsLink() {
		log("Click on the total goods link");
		getAllProductsLink().click();
	}

	public void clickGoToNextPage() {
		log("Click on next page link");
		getNextLink().click();
	}
	...
	public static boolean isElementPresent(String name) {
		boolean flag = true;
		if (Component.getDriver().findElements(By.linkText(name)).size() == 0)
			flag = false;
		return flag;
	}

	public void clickGoToPrevPage() {
		log("Click on the previous page link");
		getPrevLink().click();
	}

	public PricePage clickGoToPricePage() {
		log("Click on the price link and goto price page");
	...
	public void searchString(String search) {
		...
		log("Input request for search");
		getSearchInput().sendKeys(search);
		log("Click on the search element");
		getSearchLink().click();
	}
